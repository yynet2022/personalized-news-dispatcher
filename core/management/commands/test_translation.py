...
class Command(BaseCommand):
    ...
    """
    help = ('Test the translation functionality '
            'by connecting to the actual AI service.')
    ...
    def handle(self, *args, **options) -> None:
        ...
        self.stdout.write(
            self.style.NOTICE(f"Original text: '{text_to_translate}'"))
        self.stdout.write(
            self.style.NOTICE(f"Target language: {target_language}"))
        self.stdout.write(
            self.style.NOTICE(f"Logging level set to: {log_level}"))
        self.stdout.write("---")
        self.stdout.write("Attempting to translate...")

        try:
            translated_text = \
                translate_content(text_to_translate, target_language)

            self.stdout.write("---")
            if translated_text == text_to_translate:
                self.stdout.write(self.style.WARNING(
                    "Translation result is the same as the original text. "
                    "This might be due to a failed translation "
                    "or the text not needing translation."
                ))
            else:
                self.stdout.write(
                    self.style.SUCCESS("Translation successful!"))

            self.stdout.write(
                self.style.SUCCESS(f"Translated text: '{translated_text}'"))
