...
from .models import (  # ArXivKeywords
    CurrentKeywords,
    QuerySet,
    RelatedKeywords,
    UniversalKeywords,
)
from .services import fetch_articles_for_subscription, send_articles_email
...
class QuerySetListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return QuerySet.objects.filter(user=self.request.user).order_by(
            "source", "name"
        )
...
class _KeywordsApiView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not large_category_id:
            return JsonResponse(
                {"error": "large_category_id is required"}, status=400
            )
...
class NewsPreviewApiView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Invalid parameter format"}, status=400
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if new_articles:
            ...
            try:
                ...
                logger.error(s)
                return JsonResponse(
                    {"status": "error", "message": s}, status=500
                )
        else:
            tstr = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            msg = (
                f"{tstr}: 「{queryset.name}」に関する"
                "新しい記事は見つかりませんでした。"
            )
...
class ToggleAutoSendView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse(
                {"status": "success", "auto_send": queryset.auto_send}
            )
