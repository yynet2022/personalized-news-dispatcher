...
class Command(BaseCommand):
    help = 'Fetches news based on QuerySets and sends them to users as HTML email.'
    ...
    def process_user(self, user, site_url):
        ...
        querysets_with_articles, all_new_articles = self.fetch_all_news(user, user_querysets)
    ...
    def fetch_all_news(self, user, user_querysets):
        ...
        for queryset in user_querysets:
            ...
            new_articles_for_queryset = self.fetch_news_for_queryset(user, queryset)
    ...
    def fetch_news_for_queryset(self, user, queryset):
        ...
        for entry in feed.entries:
            article, created = Article.objects.get_or_create(url=entry.link, defaults={'title': entry.title})
            is_sent = SentArticleLog.objects.filter(user=user, article=article).exists()
            if not is_sent:
