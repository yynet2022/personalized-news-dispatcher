    ...
    for char in forbidden_chars:
        if char in value:
            raise ValidationError(
                _(
                    '"%(char)s" は使用できません。複数のキーワードをまとめたり、別名を入れたりしないでください。'
                ),
                params={"char": char},
            )
...
class CiNiiKeywords(NormalizeNameMixin, models.Model):
    ...
    description = models.CharField(
        "説明", max_length=255, blank=True, null=True
    )
...
class QuerySet(models.Model):
    ...
    query_str = models.TextField(
        "クエリ文字列", editable=False, help_text="ユーザ選択により自動生成"
    )
    ...
    universal_keywords = models.ManyToManyField(
        UniversalKeywords, blank=True, verbose_name="普遍キーワード"
    )

    current_keywords = models.ManyToManyField(
        CurrentKeywords, blank=True, verbose_name="時事キーワード"
    )

    related_keywords = models.ManyToManyField(
        RelatedKeywords, blank=True, verbose_name="関連キーワード"
    )
